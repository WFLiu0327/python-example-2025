...
def train_model(data_folder, model_folder, verbose):
    ...
    for _ in tqdm(range(100), desc="epoch", leave=False):
        model.train()
        all_pred_prob = []
        all_gt = []
        ...
        for i, (signals, labels) in enumerate(prog_iter):
                ...
                # print(f"signals device: {signals.device}, labels device: {labels.device}, outputs device: {outputs.device}")
                ...
                optimizer.step()
                step += 1
                prob_class_1 = torch.sigmoid(outputs).detach().cpu().numpy() 
                all_pred_prob.append(prob_class_1)
                print()
                print("y_true:",labels.cpu().numpy())
                print("y_pred:",(prob_class_1 > 0.5).astype(int))
        ...
        writer.add_scalar('train/challenge_score', challenge_score, _)
        writer.add_scalar('train/auroc', auroc, _)
        writer.add_scalar('train/auprc', auprc, _)
        writer.add_scalar('train/accuracy', accuracy, _)
        writer.add_scalar('train/f_measure', f_measure, _)
        ...
        with torch.no_grad():
            ...
            for i, (signals, labels) in enumerate(prog_iter):
                    ...
                    signals, labels = signals.to(device), labels.to(device)
                    outputs = model(signals).squeeze()
                    prob_class_1 = torch.sigmoid(outputs).cpu().numpy() 
                    all_pred_prob.append(prob_class_1)
                    print()
                    print("y_true:", labels.cpu().numpy())
                    print("y_pred:", (prob_class_1 > 0.5).astype(int))
        ...
        writer.add_scalar('val/challenge_score', challenge_score, _)
        writer.add_scalar('val/auroc', auroc, _)
        writer.add_scalar('val/auprc', auprc, _)
        writer.add_scalar('val/accuracy', accuracy, _)
        writer.add_scalar('val/f_measure', f_measure, _)
        # 保存最佳模型
...
def run_model(record, model, verbose):
    ...
    probability_output = torch.sigmoid(outputs).cpu().detach().numpy() 
